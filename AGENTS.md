...
- Submissions typed while the agent is stopped queue into the follow-up lane, paused, and send only on an explicit empty-composer `Enter`; `/…` commands and `!` bash keep passing straight to Pi.
